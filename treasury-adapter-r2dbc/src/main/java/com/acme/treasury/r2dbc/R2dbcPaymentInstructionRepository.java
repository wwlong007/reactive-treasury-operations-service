...
public class R2dbcPaymentInstructionRepository implements PaymentInstructionRepository {
    ...
    public Mono<PaymentInstruction> insert(PaymentInstruction p){return db.sql("insert into payment_instruction ("+COLUMNS+") values (:id,:tenant,:ref,:account,:name,:beneficiary,:amount,:currency,:status,:required,:count,:actor,:created,:updated,:version)")
