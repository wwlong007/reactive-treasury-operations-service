# ADR-0001: PostgreSQL RLS is the tenant data boundary for payment control
...
Treasury operators can reuse account and payment references across legal entities. Payment reservations and approval decisions must also remain isolated when reactive work is retried or interleaved. Repository predicates provide useful query intent but are insufficient as the final security boundary: a missing predicate in a new query must not disclose or mutate another entity's money movement data.
